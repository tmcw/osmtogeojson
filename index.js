...
osmtogeojson = function (data, options, featureCallback) {
    ...
    for (let i = 0; i < json.elements.length; i++) {
      switch (json.elements[i].type) {
        case "node":
          ...
          break;
        case "way":
          let way = _.clone(json.elements[i]);
          ...
          break;
        case "relation":
          let rel = _.clone(json.elements[i]);
          rel.members = _.clone(rel.members);
          rels.push(rel);
          let has_full_geometry =
            rel.members &&
  ...
  function _convert2geoJSON(nodes, ways, rels) {
    ...
    for (let i = 0; i < ways.length; i++) {
      ...
      if (ways[i].is_skippablerelationmember) continue; // ignore ways which are already rendered as (part of) a multipolygon
      ...
      if (!featureCallback) {
        if (way_type == "LineString") geojsonlines.push(feature);
        else geojsonpolygons.push(feature);
      } else {
...
function join(ways) {
  ...
  while (ways.length) {
    ...
    while (ways.length && !_fitTogether(_first(current), _last(current))) {
      ...
      if (!what) break; // Invalid geometry (dangling way, unclosed ring)
      ways.splice(i, 1);
